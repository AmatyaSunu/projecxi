...
function drop(ev) {
  ...
  var dropTarget = ev.target.classList.contains("kanban-block")
    ? ev.target
    : ev.target.closest(".kanban-block");
  var sourceColumnId = document
    .getElementById(data)
    .closest(".kanban-block").id;
  ...
  for (var i = 0; i < kanbanColumns.length; i++) {
    ...
    if (dropTarget.id === column.index) {
      column.count++;
      document.getElementById(column.index + "-count").textContent =
        column.count;
      break;
    }
  }
...
document
  .getElementById("create-ticket-btn1")
